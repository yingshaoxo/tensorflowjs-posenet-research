...
if "1.pose_detect_training" not in t.current_dir:
    print("You must run this script at 1.pose_detect_training folder!")
    exit()
...
like sitting.mp4 or standing.mp4

make sure the file name is meaningful
...
if len(t.run_command("ls data/*.json -l").split("\n")) == len(t.run_command("ls data/*.mp4 -l").split("\n")):
    anser = input("You alread have all data, want to collect again? (y/n)")
